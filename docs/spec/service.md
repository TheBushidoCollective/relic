...
1. **The telemetry trade.** The coarse renderer class, named as its eight values; the publishing client name; IP-correlated open activity; the optional publisher-declared plaintext title (defaulting to the source filename) stored on the relic row and served in link unfurls and the viewer document `<title>`. The class was already in the telemetry enumerations collected for the operator. What changed is its audience: it is now public metadata, served on link unfurl cards (`og:description`, `twitter:description`, and the fallback `<title>`) to anyone who fetches the link, rather than known to the operator alone. Stated as what it moves the operator and the recipient from and to: from knowing nothing to knowing what kind of thing was published, its public title when declared, and roughly how often it was fetched. Metadata, never content. The statement must note that removal (tombstone) and expiry stop serving both the title and the class, reverting the unfurl card to the constant fallback card (`A relic` with description `An encrypted file. It opens in your browser, and only someone holding the whole link, including the part after the #, can read it.`). It must also note that declining a title does not hide the class: the title is declinable, but the class is derived from the bytes and is not declinable.
