...
**Parsing is strict about fields and tolerant about kinds, and the asymmetry is the same argument run in the other direction.** An unknown field inside a known kind is refused, exactly as 3.1's envelope parser refuses one: that is the tampering and typo case, and absorbing it would silently discard what the writer meant. An unknown `kind` is not refused. It parses as `{kind: "unsupported", declared}` and the comment keeps its body, because the only thing that produces an unknown kind is a reader older than the writer, and the honest report is that a mark exists which this build cannot show. `unsupported` is a parser output and is never written: sealing one would store a comment pointing at nothing, permanently, under a key nobody can rewrite. A `display_name` that is present and neither a string nor `null` is refused, since coercing it to `null` would silently discard what the writer meant. A string posing as an `anchor`, a pin outside the unit square, and a rectangle that overhangs its edge are all refused.
