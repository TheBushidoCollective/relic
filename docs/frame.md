...
This leaks a coarse content category, a client name, IP-correlated open activity, and an optional publisher-declared plaintext title to the operator. The operator still cannot read a byte of the encrypted content, but the title is stored in the clear. What the operator now learns: the coarse renderer class, the publishing client, open counts, and the plaintext title of the content or source filename. What it buys the recipient: a legible link preview card instead of a blank card on an unfamiliar domain, which is the visual shape of a phishing link. What it costs the publisher: the title is stored server-side in plaintext and served to anyone fetching `/{id}` without the key. Declining it is a publisher's choice rather than an operator's: the publisher can pass an empty title or clear it on republish, falling back to constant fallback metadata (`A relic`). Publishers must be able to see all of it in a published privacy statement before they publish. Upload IP and timestamp are already retained for abuse response, so the IP-correlation cost is largely pre-existing.
