...
Reader-activated external links are the narrow exception, and they do not give
the rendered document network access. A trusted click on an absolute `http`,
`https`, or `mailto` destination is contained in the frame and passed to the
parent through a validated message. The parent opens it in a separate tab with
no opener or referrer. Scripted clicks and every other scheme remain blocked,
and the iframe still carries `allow-scripts` and nothing else.
...
**The success metric's renderable wedge:** Markdown (rendered, with a source toggle), code and plain text (syntax highlighted), HTML (on the usercontent origin), JSX (on the usercontent origin, executed there after transpiling on the service origin), and still images.

That set is exactly `{markdown, code, html, jsx, image}`, the renderable side of the telemetry taxonomy. This is telemetry vocabulary bounding the metric's second clause, not an exhaustive inventory of what the viewer displays: the viewer also provides an in-browser player for audio and video relics (`media`), while archives and arbitrary binaries are download-only. If the wedge slips, the taxonomy moves with it and the metric is restated. Same decision.
...
- **No first-party rendering of archives or arbitrary binaries.** Download-only in the first release, per the wedge boundary. In-browser playback for audio and video relics runs via native player elements, while seekable range-decrypted streaming media and archive browsing are out of the first release. The framing constraint stays.
